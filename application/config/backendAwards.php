 ...
 * Copyright:    Copyright (c) 2006-2023, BF2statistics.com
 * License:      GNU GPL v3
 *
 */
namespace System\BF2;

use Application\BF2\AwardCriteria;
use Application\BF2\BackendAward;
...
return array(
    ...
    new BackendAward(3270519, [
        ...
        new AwardCriteria('player_map', 'sum(time) AS result', 'map_id IN (10,11,110)',
            ...
            {
                $time = (int)$row['result'];
                return ($timesAwarded == 0 && $time >= 180000);
            }
        ),
    ]),
    ...
    new BackendAward(3271401, [
        ...
        new AwardCriteria('player_map', 'sum(time) AS result', 'map_id IN (200,201,202)',
            ...
            {
                $time = (int)$row['result'];
                return ($timesAwarded == 0 && $time >= 90000);
            }
        ),
    ]),
