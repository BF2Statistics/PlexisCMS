...
/**
 * A class responsible for calculating the next obtainable ranks for a player
 * within the Battlefield 2 game. It provides methods to determine
 * eligible rank promotions based on player statistics and required awards.
 */
class RankCalculator
{
    ...
    public function __construct()
    {
        ...
        $this->connection = DbFactory::GetConnection('stats');

        // Load ranks
        $this->ranks = include Path::Combine(APP_DIR, 'config', 'ranks.php');
    }
     ...
     * @throws \Exception
     */
    public function getNextRanks(int $playerId, int $count): array|bool
    {
        // sanitize
        ...
        $player = $this->connection->query($query)->fetch();
        if (empty($player))
            return false;
            ...
            foreach ($nextPromoRanks as $rankId => $rank)
            {
                ...
                if (!empty($rank['has_awards']))
                {
                    foreach ($rank['has_awards'] as $awardId => $level)
                    {
                        ...
                        if ($this->connection->query($query)->fetchColumn(0) == 0)
                        {
     ...
     * @return array
     */
    protected function getNextRankUps(int $rank_id): array
    {
        $ranks = [];
     ...
     * @throws \Exception
     */
    protected function generateMissingDesc(string $rankName, bool $prevRank)
    {
        // Message
        $message = ($prevRank)
